...
export default async function DashboardLayout({
  ...
  const menuItems = [
    { name: 'Dashboard', icon: <IconLayoutDashboard size={20} />, href: '/dashboard', active: true },
    { name: 'Produtores & Clientes', icon: <IconUsers size={20} />, href: '#', active: false },
    { name: 'Vendas & Negócios', icon: <IconReceipt2 size={20} />, href: '#', active: false },
    { name: 'Estoques & Insumos', icon: <IconBuildingWarehouse size={20} />, href: '#', active: false },
    { name: 'Configurações', icon: <IconSettings size={20} />, href: '#', active: false },
  ];
    ...
    <div className="min-h-screen bg-[#030712] text-slate-100 flex flex-col md:flex-row relative">
            ...
            {menuItems.map((item, idx) => (
              <a
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${
                  item.active
                    ? 'bg-emerald-600/10 border border-emerald-500/25 text-emerald-400 font-semibold'
      ...
      <main className="flex-1 flex flex-col min-w-0 z-10 overflow-y-auto max-h-screen">
        ...
        <header className="glass-panel border-b border-slate-900/40 px-6 py-4 flex items-center justify-between shrink-0">
          <div>
            <h1 className="text-lg font-bold tracking-tight text-slate-100">Painel Principal</h1>
          </div>

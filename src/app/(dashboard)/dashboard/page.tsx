...
export default async function DashboardPage() {
  ...
  // Estatísticas simuladas premium
  const stats = [
    {
    ...
    },
    {
      ...
      value: '48 ativos',
      description: 'novos contatos registrados',
    ...
    },
    {
      ...
      trend: { value: '4.3%', isPositive: false }, // Tendência negativa de pendência é positiva (menos pendência)
    },
  ];

  // Lista de negociações simuladas
  const deals = [
    {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-400 text-sm">
          Aqui está o resumo operacional das vendas e insumos agrícolas de hoje.
        </p>
      </div>
      ...
      {/* Grid Secundário: Tabela de Negociações */}
      <div className="glass-panel p-6 rounded-2xl">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-bold text-slate-100">Negociações Recentes</h3>
            <p className="text-xs text-slate-400 mt-1">Status de propostas e vendas ativas no campo.</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-slate-900/60">
              {deals.map((deal, idx) => (
                <tr key={idx} className="hover:bg-slate-900/20 transition-all duration-200 group">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center justify-center">
                      <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full border ${deal.statusColor}`}>
                        {deal.iconStatus}
                        {deal.status}
                      </span>
                    </div>
